...
func (a *HubApp) registerProvisioningMutations() error {
	...
	if err := a.mux.HandleFunc("default", "create_agent", a.handleCreateAgent,
		...
		app.Mutation(),
		app.Desc("Provision a new agent identity in the Agent DB and mint its one-shot bootstrap secret (spec-hub-side HB2). agent_type_id must exist. All args are required by the framework: pass hugr_role='' to auto-create a dedicated per-agent role 'agent:<agent_id>' carrying the isolation floor — grant it data-source/function access afterwards with standard core.insert_role_permissions mutations. A NON-empty hugr_role is used AS IS and is NOT floored by the hub — it must already exist, and the admin owns ensuring it is appropriately floored (create it with create_agent_role, or pass 'agent' to share the pre-floored base role). A privileged role (e.g. admin) is honoured as the admin's deliberate choice — provisioning an admin-privileged agent is intentional, not blocked. owner_user_id='' skips the owner grant, config_override='{}' means no per-instance overrides. Returns {agent_id, status, secret, expires_at}. Admin only."),
	); err != nil {
		return err
	}
...
func (a *HubApp) handleCreateAgent(w *app.Result, r *app.Request) error {
	...
	if role == "" {
		...
		// functions) onto it afterwards with standard core.insert_role_permissions
		// mutations. Pass hugr_role="agent" to share the base role instead
		// (design/008 spec-hub-gateway §12).
		role = perAgentRoleName(id)
	}
...
func (a *HubApp) handleUpdateAgent(w *app.Result, r *app.Request) error {
	...
	newRole := strings.TrimSpace(r.String("hugr_role")) // arg is hugr_role; column is role
	if newRole != "" {
		data["role"] = newRole
	}
